...
export function openAssignEanSheet(ean, products, onAssigned) {
  ...
  const render = () => {
    ...
    openSheet(`
      ...
      <input id="assign-q" type="search" autocomplete="off" value="${query}"
      ...
      ${matches.length === 0 && q.length > 1
        ? `<div style="text-align:center;padding:20px;color:var(--text3);font-size:0.82rem">Sin resultados</div>`
        : matches.length > 0
          ? `<div class="prod-list">
    ...
    document.getElementById('assign-q').addEventListener('input', e => {
      query = e.target.value;
      render();
    });
    document.getElementById('assign-q').focus();

    document.querySelectorAll('.assign-pick').forEach(el => {
